...
class RabbitMQClient {
  ...
  async connect(url = process.env.RABBITMQ_URL || 'amqp://localhost:5672') {
    try {
      if (this.connection) {
        console.log('[Schedule RabbitMQ] Already connected');
        return;
      }
      ...
      console.log('[Schedule RabbitMQ] Connected successfully');
  ...
  async publishToQueue(queueName, message) {
    try {
      ...
      console.log(`[Schedule RabbitMQ] Published to queue ${queueName}:`, message);
      return true;
    } catch (error) {
  ...
  async consumeQueue(queueName, handler) {
    try {
      ...
      channel.consume(queueName, async (msg) => {
        if (msg) {
          try {
            ...
            console.log(`[Schedule RabbitMQ] Received from ${queueName}:`, data);
            ...
            console.error(`[Schedule RabbitMQ] Error processing message from ${queueName}:`, error);
      ...
      console.log(`[Schedule RabbitMQ] Consuming queue: ${queueName}`);
    } catch (error) {
